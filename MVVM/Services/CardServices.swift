...
final class CardServices {
    ...
    static func getAllCards(completion: @escaping (Result<[Card], Error>) -> Void) {
        guard Reachability.isConnectedToNetwork(), let url = URL(string: Endpoint.cardList.url) else { completion(.failure(CustomError.noConnection)); return }
        ...
        URLSession.shared.dataTask(with: request) { data, _, error in
            ...
            guard let data = data else { completion(.failure(CustomError.noData)); return }
